...
class LinkedBST(AbstractCollection):
    ...
    def isBalanced(self):
        ...
        def recurse():
            '''
            Return count vartexes
            :return:int
            '''
            count = 0
            iterator = self.inorder()
            for i in iterator:
                count += 1
            return count

        n = recurse()
        if self.height() < 2 * log(2 * (n + 1)) - 1:
            return True
        return False
    ...
    def rebalance(self):
        ...
        def recurse(items):
            if not items:
                return None
            mid = len(items) // 2
            return BSTNode(items[mid], left=recurse(items[:mid]), right=recurse(items[mid + 1:]))

        self._root = recurse(items)
